...
# model setting
sys.path.append('.') # 
from metainvnet.utils import ssim_loss
from metainvnet.model import l2loss_mean
...
from metainvnet.model.net import MetaInvNet
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.real_n_views = 60

        if args.n_views != 60:
            self.real_n_views = args.n_views
            args.n_views = 60

        if args.n_views != 60:
            module = module+"_n_views"+str(args.n_views)

        # if args.real_n_views != 60:
            # self.real_n_views = args.n_views
            # args.n_views = self.real_n_views
        ...
        radon_view = args.real_n_views
        self.n_views = args.real_n_views
        img_width = 256
        options = torch.tensor([args.real_n_views, 640, img_width, img_width, 0.006641, 0.012858, 0, 0.009817477*640/radon_view,
                5.95, 4.906, 0, 0]).cuda()
        ...
        self.model = MetaInvNet(args, options)
        ...
        self.optimizer = torch.optim.Adam(self.model.net.parameters(), lr=self.lr)
        ...
        self.dataset_sup, self.val_loader_sup = get_tta_ct_dataset(args.pretrain_dataset)
        ...
        self.model.net.to(device, dtype=torch.float)
...
class Trainer:
    def __init__(self, dataset_sup: DataLoader, dataset_unsup: DataLoader, modelset: ModelSet):
        ...
        self.criterion = modelset.loss_fun
        


        # define a MPG noise model
        n_views = modelset.real_n_views
        I0 = 1e5
        noise_sigam = 30
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train_rec = self.model.tr_model(proj, fbp_img)
    ...
    def _run_epoch_rotcsmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            # x_train_org = out['zero_filling']
            # x_train_rec_1 = self.model(x_train_org)
            # loss1 = self.criterion(x_train_rec_1, y_train) 

            # +  self.sobel_loss(x_train_rec_1, y_train) \
