...
from model_backbones.recon_net import Unet, UnetCT
from ct_dependencies.REI_dependencies.eiunet import UNet
...
from contributions.ortho import ortho_reg, my_ortho_norm, ct_ortho_reg
from ct_dependencies.EI_dependencies.ct import CT
from ct_dependencies.EI_dependencies.rotate import Rotate
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        if args.n_views != 60:
            module = module+"_n_views"+str(args.n_views)
        ...
        self.ckpt = './model_zoo/ct_{}/{}_{}_{}.pth'.format(args.shift, args.pretrain_dataset, args.supervised_dataset_name, module)
        args.save_path = self.ckpt
        img_size = (256, 256)


        # options =  [60, 736, 256, 256, 0.006641, 0.012858, 0, 0.104719, 5.95, 4.906, 0, 0]
        radon_view = args.n_views
        ...
        # options = options.to(device, dtype=torch.float)
        self.options = options

        self.model = LEARN(options, block_num=10)
        # self.model = UNet()
        ...
        train_module = 'our_s1_train.py'
        training_save_path = './model_zoo/ct_{}/{}_{}_{}.pth'.format(args.shift, args.pretrain_dataset, args.supervised_dataset_name, train_module)

        self.model.load_state_dict(
            torch.load(training_save_path))
        print("Finished load model parameters! in {}".format(training_save_path))
        ...
        self.dataset_sup, self.val_loader_sup = get_tta_ct_dataset(args.supervised_dataset_name)


        self.dataset_unsup, self.val_loader_unsup = get_tta_ct_dataset(args.supervised_dataset_name)
...
class Trainer:
    def __init__(self, dataset_sup: DataLoader, dataset_unsup: DataLoader, modelset: ModelSet):
        ...
        # self.physics = CT(256, n_views, circle=False, device=device, I0=I0,
                            # noise_model=noise_model)
        self.physics = CT(256, n_views, circle=False, device=device)
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            # self._validate(epoch, dataloader_sup)


            self._run_epoch_unsup_ei(epoch, self.val_loader_sup)
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            # proj = ctlib_v2.projection(y_train, self.options)
            # fbp_img = ctlib_v2.fbp(proj, self.options)

            proj = self.physics.A(y_train)
            fbp_img = self.physics.A_dagger(proj)

            x_train_rec = self.model(fbp_img, proj)

            # meas1 = self.physics.A(x_train_rec)

            # loss_mc = self.criterion(meas1, proj)


            # proj_mc = ctlib_v2.projection(x_train_rec, self.options)
            # loss_mc = self.criterion(proj, proj_mc)
            ...
            if step == 1:
                ...
                self.plotter.image('zf', epoch, fbp_img)
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            meas0 = self.physics.A(y_train)
            # s_mpg = torch.log(physics.I0 / meas0)
            fbp_mpg = self.physics.A_dagger(meas0)
            x1 = self.model(fbp_mpg, meas0)
            meas1 = self.physics.A(x1)

            loss_mc = self.criterion(meas1, meas0)
            # EI: x2, x3
            x2 = self.transform.apply(x1)
            meas2 = self.physics.A(x2)
            # s2 = torch.log(physics.I0 / meas2)
            fbp_2 = self.physics.A_dagger(meas2)
            x3 = self.model(fbp_2, meas2)
            loss_eq = self.criterion(x3, x2)
            
            # loss_w = 1e-8 * ct_ortho_reg(self.model, "cuda")

            # loss = loss_mc + 100* loss_eq
            # loss_w = 0
            loss = 1e-5*(loss_mc + 100* loss_eq)
            ...
            img_x = y_train.detach().cpu().numpy()[0, 0, ...]
            img_out = rec_img.detach().cpu().numpy()[0, 0, ...]
            ...
            # if step:
                ...
                self.plotter.image('rec_img', epoch, rec_img)
                self.plotter.image('ei_itput', epoch, fbp_eq_img)
                self.plotter.image('ei_output', epoch, rec_eq_img)

                self.plotter.image('proj_mc', epoch, proj_mc)
                self.plotter.image('proj', epoch, proj)
    ...
    def _run_epoch_rotcsmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            rotate_y_train = self.transform.apply(y_train)
            meas2 = self.physics.A(rotate_y_train)
            fbp_2 = self.physics.A_dagger(meas2)
            x_train_rec = self.model(fbp_2, meas2)

            # loss = 1e-5 * self.tv_loss(x_train_rec)
            loss2 =self.criterion(x_train_rec, rotate_y_train) 

            # +  self.sobel_loss(x_train_rec, rotate_ytrain) \
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                ...
                for i in range(0, y_test.shape[0]): # val dataloader batch
                    ...
                    if test_num == 30:
                        ...
                        self.plotter.image('val_gt', epoch, y_test.detach().cpu())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="ixi_t2_periodic_slight",
        help="which dataset")
    parser.add_argument(
