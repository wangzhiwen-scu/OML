...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        if args.acc == 2:
            module = module+"_acc"+str(args.acc)
        ...
        options =  torch.tensor([60, 736, 256, 256, 0.006641, 0.012858, 0, 0.104719, 5.95, 4.906, 0, 0]).cuda()
        radon_view = 60
        img_width = 256
...
class Trainer:
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            meas0 = ctlib_v2.projection(y_train, self.options)
            # fbp_mpg = self.physics.A_dagger(meas0)
            fbp_mpg = ctlib_v2.fbp(meas0, self.options)

            x1 = self.model(fbp_mpg, meas0)
            # meas1 = self.physics.A(x1)
            meas1 = ctlib_v2.projection(x1, self.options)

            loss_mc = self.criterion(meas1, meas0)
            # EI: x2, x3
            x2 = self.transform.apply(x1)
            
            # x2 = self.physics.A(x2)
            meas2=ctlib_v2.projection(x2, self.options)
            # fbp_2 = self.physics.A_dagger(meas2)
            fbp_2=ctlib_v2.fbp(meas2, self.options)
