...
class Trainer:
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            proj = ctlib_v2.projection(y_train, self.options)
            fbp_img = ctlib_v2.fbp(proj, self.options)
            rec_img = self.model(fbp_img, proj)
            proj_mc = ctlib_v2.projection(rec_img, self.options)
            ...
            for theta in theta_list:
                rotate_x_train_rec_temp = torch_rotate(rec_img, int(theta))
                rotate_x_train_rec.append(rotate_x_train_rec_temp)
            rotate_x_train_rec_input = torch.cat(rotate_x_train_rec, dim=0)

            proj_eq = ctlib_v2.projection(rotate_x_train_rec_input, self.options)
            fbp_eq_img = ctlib_v2.fbp(proj_eq, self.options)
            rec_eq_img = self.model(fbp_eq_img, proj_eq)

            loss_eq =self.criterion(rec_eq_img, rotate_x_train_rec_input) 
