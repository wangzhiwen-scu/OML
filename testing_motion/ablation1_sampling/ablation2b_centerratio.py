...
def test(ourtta):
    ...
    args.ablation_which = 'ablation3'
  

    args.ablation_var = '24'
    modelset = Model(args, test=True)
    model_24 = modelset.model  
    model_24.eval()

    args.ablation_var = '34'
    modelset = Model(args, test=True)
    model_34 = modelset.model  
    model_34.eval()

    args.stage = '1'
    modelset = OursModel(args, test=True)
    model_44 = modelset.model
    model_44.eval()

    args.ablation_var = '54'
    modelset = Model(args, test=True)
    model_54 = modelset.model  
    model_54.eval()

    args.ablation_var = '64'
    modelset = Model(args, test=True)
    model_64 = modelset.model  
    model_64.eval()

    args.ablation_var = '74'
    modelset = Model(args, test=True)
    model_74 = modelset.model  
    model_74.eval()

    args.ablation_var = '84'
    modelset = Model(args, test=True)
    model_84 = modelset.model  
    model_84.eval()
    ...
    df = {'24': None, '34': None, '44': None, '54': None, '64':None, '74':None, '84':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_24(inputs)
            imgs['24'] = tensor2np(pred_results)

            pred_results = model_34(inputs)
            imgs['34'] = tensor2np(pred_results)

            pred_results = model_44(inputs)
            imgs['44'] = tensor2np(pred_results)

            pred_results = model_54(inputs)
            imgs['54'] = tensor2np(pred_results)

            pred_results = model_64(inputs)
            imgs['64'] = tensor2np(pred_results)

            pred_results = model_74(inputs)
            imgs['74'] = tensor2np(pred_results)

            pred_results = model_84(inputs)
            imgs['84'] = tensor2np(pred_results)
