...
def test(ourtta):
    ...
    model_20 = modelset.model  
    model_20.eval()

    args.ablation_var = '30'
    modelset = Model(args, test=True)
    model_30 = modelset.model  
    model_30.eval()

    args.ablation_var = '40'
    modelset = Model(args, test=True)
    model_40 = modelset.model  
    model_40.eval()

    args.ablation_var = '50'
    modelset = Model(args, test=True)
    model_50 = modelset.model  
    model_50.eval()

    # args.supervised_dataset_name = 'ixi_t1_periodic_slight'
    args.stage = '1'
    modelset = OursModel(args, test=True)
    model_60 = modelset.model
    model_60.eval()

    args.ablation_var = '70'
    modelset = Model(args, test=True)
    model_70 = modelset.model  
    model_70.eval()

    args.ablation_var = '80'
    modelset = Model(args, test=True)
    model_80 = modelset.model  
    model_80.eval()

    args.ablation_var = '90'
    modelset = Model(args, test=True)
    model_90 = modelset.model  
    model_90.eval()
    ...
    df = {'20': None, '30': None, '40': None, '50': None, '60':None, '70':None, '80':None, '90':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            pred_results = model_20(inputs)
            imgs['20'] = tensor2np(pred_results)

            pred_results = model_30(inputs)
            imgs['30'] = tensor2np(pred_results)

            pred_results = model_40(inputs)
            imgs['40'] = tensor2np(pred_results)

            pred_results = model_50(inputs)
            imgs['50'] = tensor2np(pred_results)

            pred_results = model_60(inputs)
            imgs['60'] = tensor2np(pred_results)

            pred_results = model_70(inputs)
            imgs['70'] = tensor2np(pred_results)

            pred_results = model_80(inputs)
            imgs['80'] = tensor2np(pred_results)

            pred_results = model_90(inputs)
            imgs['90'] = tensor2np(pred_results)
            
            # if ourtta == 'ixi_t1_periodic_slight_rl':
            #     for key in keys_list:
            #         imgs[key] = tensor2np(torch_rotate(recon_bsa, -90))
