...
# model setting
sys.path.append('.') # 
from data.dataset import get_h5py_supervised_dataset, get_h5py_unsupervised_dataset, DIV2KDataset, get_tta_noise_dataset as get_tta_dataset
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        # self.ckpt = './model_zoo/mri_{}/{}_{}_{}.pth'.format(args.shift, args.pretrain_dataset, args.supervised_dataset_name, module)
        self.ckpt = './model_zoo/mri_{}/{}_{}_{}_sigma_{}.pth'.format(args.shift, args.pretrain_dataset, args.supervised_dataset_name, module, args.sigma)
        print(self.ckpt)
        args.save_path = self.ckpt
        img_size = (256, 256)
        # img_size = (240, 240)
        self.sample_net = Hand_Tailed_Mask_Layer(args.acc, 'cartesian', img_size, resize_size=img_size).to(device, dtype=torch.float)
        ...
        # self.model = Unet(in_ch=2, out_ch=2)

        # for m in self.model.modules():
        #     if isinstance(m, (nn.Conv2d, nn.Linear)):
        #         nn.init.orthogonal(m.weight)

        # training_save_path = 'model_zoo/exp1/fastmri_brain_t1_MR_ART_our_s1_train.py_None.pth'
        # pretrain_dataset = "fastmri_knee_t1"

        # pretrain_dataset = "fastmri_brain_t1"

        train_module = 'main_ttt_train.py'
        # if args.acc != 4:
        if args.shift == "ratio":
            train_module = train_module+"_acc"+str(10)

        training_save_path = './model_zoo/mri_{}/{}_{}_{}.pth'.format(args.shift, args.pretrain_dataset, args.supervised_dataset_name, train_module)
        ...
        self.dataset_sup, self.val_loader_sup = get_tta_dataset(args.supervised_dataset_name, args.sigma)
...
class Trainer:
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            y_train_with_noise = with_motion.to(device, dtype=torch.float)
            y_train = still.to(device, dtype=torch.float)
            ...
            out_1st = self.sample_net(y_train_with_noise)
            ...
            loss_mc = 1e-5*self.criterion(k_measure, k_mc)
            # loss_mc = 1e-1*self.criterion(k_measure, k_mc)
            ...
            x_train_rec = x_train_rec_mc.detach()
    ...
    def _validate(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                y_test_with_noise = withmotion_test.to(device, dtype=torch.float)
                y_test = still_test.to(device, dtype=torch.float)

                out_test = self.sample_net(y_test_with_noise)
