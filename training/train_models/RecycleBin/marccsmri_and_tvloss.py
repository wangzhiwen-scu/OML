...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            residules = self.model(x_train)

            loss = self.criterion(residules, x_train-y_train) # because x_train have more energy.

            x_train_rec = x_train - residules
    ...
    def _run_epoch_unsup(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            residuals_test = self.model(x_train)
            x_train_rec = x_train-residuals_test
