...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.dataset_sup, self.val_loader_sup = get_h5py_supervised_dataset(args.supervised_dataset_name, tta_testing=True, bezier=True)
...
class Trainer:
    ...
    def train(self):
        ...
        pretrain_path = './model_zoo/exp1/ixi_t1_periodic_slight_MR_ART_csmri_ei.py_None.pth'
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            with torch.no_grad():
                ...
                x_train = out['complex_abs']
                pesudo_label_1 = self.model_ema(x_train_org).detach()
                ...
                pesudo_label = pesudo_label_1
                # or do not use samplenet
            self.model.train()

            # x_train_for_train = pesudo_label
            
            epoch_idx = torch.randperm(40)[:pesudo_label.shape[0]]  # total 40 cartesian, choose batch numer.
            self.sample_net.epoch_idx = epoch_idx

            out_1st = self.sample_net(pesudo_label)
            ...
            # loss_mc = self.criterion(k_measure, k_mc) + self.criterion(pesudo_label, x_train_rec_mc)
