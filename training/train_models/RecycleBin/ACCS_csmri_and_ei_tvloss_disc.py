...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            # identity loss
            same_B = self.model(y_train)
            loss_identity_B = self.criterion_identity(same_B, y_train)*0.1

            # fake_B = x_train_rec
            # pred_fake = self.Disc_B(fake_B)
            # target_real = Variable(self.Tensor(*(pred_fake.shape)).fill_(1.0), requires_grad=False)
            # loss_GAN_AB = self.criterion_GAN(pred_fake, target_real)

            # loss = loss + loss_identity_B + loss_GAN_AB
            loss = loss + loss_identity_B
            ...
            # self._run_discriminator_B(real_B=y_train, fake_B=fake_B)
    ...
    def _run_discriminator_B(self,real_B, fake_B):            ##### Discriminator A #####
        ...
        target_real = Variable(self.Tensor(*(pred_real.shape)).fill_(1.0), requires_grad=False)
        loss_D_real = self.criterion_GAN(pred_real, target_real)
        # fake loss
        fake_B = self.fake_B_buffer.push_and_pop(fake_B)
        pred_fake = self.Disc_B(fake_B.detach())

        target_fake = Variable(self.Tensor(*(pred_fake.shape)).fill_(0.0), requires_grad=False)
...
def train():
    ...
    dataloader_sup = DataLoader(modelset.dataset_sup, 
                            ...
                            shuffle=True,
                            num_workers=4, drop_last=True)  # 增加GPU利用率稳定性。
