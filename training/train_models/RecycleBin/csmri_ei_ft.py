...
class Trainer:
    ...
    def train(self):
        ...
        load_path_saved = './model_zoo/exp1/ixi_t1_periodic_slight_MR_ART_csmri_ei.py_None.pth'

        if os.path.exists(
                load_path_saved) and args.load_model:
            self.model.load_state_dict(
                torch.load(load_path_saved))
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            epoch_idx = torch.randperm(40)[:x_train_org.shape[0]]  # total 40 cartesian, choose batch numer.
            self.sample_net.epoch_idx = epoch_idx

            out_1st = self.sample_net(x_train_org)
