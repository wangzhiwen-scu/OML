...
from model_backbones.dota_mri import DOTA
from config.dota_mri_config import Config
from utils.train_metrics import dcloss, imgloss
...
class Trainer:
    def __init__(self, dataset_sup: DataLoader, dataset_unsup: DataLoader, modelset: ModelSet):
        ...
        self.img_loss = imgloss().cuda()
        self.dc_loss = dcloss().cuda()
        self.tv_loss = TVLoss()
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            output = self.sample_net(x_train)

            # output = {'masked_k': undersample, 'complex_abs': complex_abs, 'zero_filling': uifft, 'batch_trajectories': batch_trajectories}

            x_train_rec = self.model(output['masked_k'], output['batch_trajectories'])

            # img_part = torch.zeros_like(y_train).to(device, dtype=torch.float)
            # y_train = torch.cat([x_train, img_part], dim=1)
            dc_loss = self.dc_loss(x_train_rec, output['masked_k'], output['batch_trajectories'])

            x_train_rec = torch.norm(x_train_rec, dim=1, keepdim=True)
            loss = self.img_loss(x_train_rec, x_train) + dc_loss

            zero_filling = output['complex_abs']
            ...
            img_x = x_train.detach().cpu().numpy()[0, 0, ...]
            ...
            if step == 1:
                ...
                self.plotter.image('still_sup', epoch, x_train)
                self.plotter.image('zero_filling', epoch, zero_filling)
    ...
    def _validate_ssid(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                ...
                test_output = self.sample_net(x_test)
                x_test_rec1 = self.model(test_output['masked_k'], test_output['batch_trajectories'])
