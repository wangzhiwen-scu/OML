...
from model_backbones.recom_k_dual import SequentialASLDualDo as SequentialASL
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.model = SequentialASL(num_step=2, shape=[240,240],  sparsity=args.rate, mini_batch=args.batch_size, args=args) 
...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            pred_results = self.model(x_train)
            x_train_rec, new_mask = (pred_results['output'], pred_results['mask'])
    ...
    def _validate(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                ...
                pred_results = self.model(x_test)
                x_test_rec, new_mask = (pred_results['output'], pred_results['mask'])
