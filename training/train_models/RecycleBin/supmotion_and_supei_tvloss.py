...
# model setting
sys.path.append('.') # 
from data.dataset import get_h5py_supervisedSlices_dataset, get_h5py_unsupervisedSlices_dataset
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.sample_net = Hand_Tailed_Mask_Layer(0.33, 'cartesian', (240, 240)).to(device, dtype=torch.float)
        self.model = Unet(in_ch=3).to(device, dtype=torch.float) # for 3 slices
        ...
        self.dataset_sup, self.val_loader_sup = get_h5py_supervisedSlices_dataset(args.supervised_dataset_name)
        self.dataset_unsup, self.val_loader_unsup = get_h5py_unsupervisedSlices_dataset(args.unsupervised_dataset_name)
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            # self._run_epoch_sup_ei(epoch, dataloader_sup)
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname, with_motion_down, witho_motion_up) in loop:
            step += 1
            ...
            with_motion_down = with_motion_down.to(device, dtype=torch.float)
            witho_motion_up = witho_motion_up.to(device, dtype=torch.float)

            out = self.sample_net(x_train)
            x_train = out['complex_abs']

            x_train_input = torch.cat([with_motion_down,x_train, witho_motion_up], dim=1)

            x_train_rec = self.model(x_train_input)
    ...
    def _run_epoch_sup_motion(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname, with_motion_down,witho_motion_up) in loop:
            ...
            loss = 1 * self.criterion(x_train_rec, y_train)
            ...
            if step == 1:
                ...
                self.plotter.image('still_motion', epoch, y_train)
                self.plotter.image('with_motion_motion', epoch, x_train)
                self.plotter.image('rec_motion', epoch, x_train_rec)
    ...
    def _run_epoch_sup_ei(self, epoch, dataloader):
        step = 0
        epoch_loss = 0
        self.metrics.ssims, self.metrics.psnrs = [], []
        learning_rate = self.optimizer.state_dict()['param_groups'][0]['lr'] # ! parameter group 0, group 1, ...
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            out = self.sample_net(x_train_rec)
            out = out['complex_abs']
            loss1 = self.criterion(out, x_train)

            rotate_ytrain = torch_rotate(x_train_rec, 90)
    ...
    def _validate(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname, with_motion_down_vali, with_motion_up_vali) in val_loop: #  增加GPU 利用率
                x_test = withmotion_test.to(device, dtype=torch.float)
                y_test = still_test.to(device, dtype=torch.float)
                with_motion_down_vali = with_motion_down_vali.to(device, dtype=torch.float)
                with_motion_up_vali = with_motion_up_vali.to(device, dtype=torch.float)

                x_test_input = torch.cat([with_motion_down_vali,x_test, with_motion_up_vali], dim=1)

                x_test_rec = self.model(x_test_input)
