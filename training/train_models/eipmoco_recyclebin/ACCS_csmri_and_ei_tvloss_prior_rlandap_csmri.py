...
from layers.mask_layer import Hand_Tailed_Mask_LayerAP_RL
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.sample_net = Hand_Tailed_Mask_LayerAP_RL(0.33, 'cartesian', (240, 240), resize_size=[240,240]).to(device, dtype=torch.float)
        # self.model = Unet().to(device, dtype=torch.float)
...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            RL_seed = torch.rand(1)
            out = self.sample_net(x_train,RL_seed=RL_seed)
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            RL_seed = torch.rand(1)
            out = self.sample_net(y_train,RL_seed=RL_seed)
            x_train = out['complex_abs']
            x_train_rec = self.model(x_train)
            ...
            out = self.sample_net(rotate_ytrain,RL_seed=RL_seed)
