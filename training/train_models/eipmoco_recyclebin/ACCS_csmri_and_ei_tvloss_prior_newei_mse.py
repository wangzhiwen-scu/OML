...
class Trainer:
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train, k_measure = out['complex_abs'], out['undersample']
            x_train_rec = self.model(x_train)

            y1_temp = self.sample_net(x_train_rec)
            k_mc = y1_temp['undersample']
            ...
            for theta in theta_list:
                rotate_x_train_rec_temp = torch_rotate(x_train_rec, theta)
            ...
            loss_eq =self.criterion(x_train_rec, rotate_x_train_rec) +  self.sobel_loss(x_train_rec, rotate_x_train_rec) \
                    + 1e-3*self.vgg_loss(x_train_rec, rotate_x_train_rec, feature_layers=[0, 1, 2, 3], style_layers=[])
