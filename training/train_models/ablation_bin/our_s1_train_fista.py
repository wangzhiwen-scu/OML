...
# model setting
sys.path.append('.') # 
from data.dataset import get_h5py_supervised_dataset, get_h5py_unsupervised_dataset, DIV2KDataset, get_tta_dataset
...
# from model_backbones.ista_net import ISTANetplus, ista_net_loss
from model_backbones.fista_net import FISTANetPlus, fista_net_loss, fista_net_loss_l1
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        img_size = (256, 256)
        # img_size = (240, 240)
        self.sample_net = Hand_Tailed_Mask_Layer(0.33, 'cartesian', img_size, resize_size=img_size).to(device, dtype=torch.float)
        # self.model = Unet().to(device, dtype=torch.float)
        ...
        self.model = FISTANetPlus(LayerNo=7)
        ...
        self.dataset_sup, self.val_loader_sup = get_tta_dataset(args.supervised_dataset_name)
...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            batch_trajectories = out['batch_trajectories']
            ...
            x_train_rec, layers_sym, layers_st = self.model(x_train, out['batch_trajectories'])
            ...
            loss = fista_net_loss(x_train_rec, y_train, layers_sym, layers_st)
            ...
            if step == 1:
                ...
                self.plotter.mask('mask', epoch, batch_trajectories)
    ...
    def _run_epoch_csmri_and_rotmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train = torch.norm(x_train, dim=1, keepdim=True)
            y_train = torch.norm(y_train, dim=1, keepdim=True)
            x_train_rec_odegree = torch.norm(x_train_rec, dim=1, keepdim=True)

            loss1 = self.criterion(x_train_rec_odegree, y_train)
            ...
            for theta in theta_list:
                rotate_y_train_temp = torch_rotate(x_train_rec_odegree, int(theta))
            ...
            img_x = y_train.detach().cpu().numpy()[0, 0, ...]
            img_out = x_train_rec_odegree.detach().cpu().numpy()[0, 0, ...]
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train_rec_mc, layers_sym_1, layers_st_1 = self.model(x_train, out_1st['batch_trajectories'])

            # x_train = torch.norm(x_train, dim=1, keepdim=True)
            # y_train = torch.norm(y_train, dim=1, keepdim=True)
            # x_train_rec = torch.norm(x_train_rec_mc, dim=1, keepdim=True)
            ...
            loss_mc = fista_net_loss(k_measure, k_mc, layers_sym_1, layers_st_1)
            ...
            # x_train_rec = self.model(out['masked_k'] , x_train, out['batch_trajectories'])
            x_train_rec, layers_sym_2, layers_st_2 = self.model(x_train, out['batch_trajectories'])
            ...
            loss_eq = fista_net_loss(x_train_rec, rotate_x_train_rec_input, layers_sym_1, layers_st_1)
    ...
    def _run_epoch_rotcsmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            x_train_rec = self.model(x_train, out['batch_trajectories'])
            ...
            loss = fista_net_loss(x_train_rec, rotate_y_train, layers_sym, layers_st)
    ...
    def _validate(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                ...
                x_test_rec, layers_sym, layers_st = self.model(x_test, out_test['batch_trajectories'])
                ...
                # x_train_rec, layers_sym = self.model(x_train, out['batch_trajectories'])

                # x_train = torch.norm(x_train, dim=1, keepdim=True)
                # y_train = torch.norm(y_train, dim=1, keepdim=True)
                # x_train_rec = torch.norm(x_train_rec, dim=1, keepdim=True)

                # loss = ista_net_loss(x_train_rec, layers_sym, y_train)
