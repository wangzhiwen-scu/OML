...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        # training_save_path = './model_zoo/exp1/{}_{}_{}_{}.pth'.format(pretrain_dataset, args.unsupervised_dataset_name, train_module, str(args.headmotion))
        ...
        self.dataset_sup, self.val_loader_sup = get_tta_dataset(args.pretrain_dataset)
...
class Trainer:
    ...
    def _run_epoch_csmri(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            y_train_img_part = torch.zeros_like(y_train)
            y_train_cplx = torch.cat([y_train, y_train_img_part], dim=1)

            loss = self.criterion(x_train_rec, y_train_cplx)
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            # loss_img = torch.norm(x_train_rec_mc-y_train, 1) / torch.norm(y_train.detach(), p=1)
            # loss_mc = torch.norm(k_measure-k_mc, 1) / torch.norm(k_measure.detach(), p=1)
            # loss = 1e3*self.criterion(k_measure, k_mc) + self.criterion(x_train_rec_mc, y_train)
            loss = 1e-5*self.criterion(k_measure, k_mc)



            # loss = loss_mc + loss_img
