...
from model_backbones.nafnet import NAFNet, MRINAFNet  # nice net, but overfitting
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        # self.model = MRINAFNet(img_channel=2)
        ...
        # if args.supervised_dataset_name == "ixi_t1_periodic_slight_sagittal":
...
class Trainer:
    ...
    def _run_epoch_unsup_ei(self, epoch, dataloader):
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            # x_train_rec_mc = torch.norm(x_train_rec_mc, dim=1, keepdim=True)
            ...
            loss = weight* (loss_mc + loss_eq) + loss_w
