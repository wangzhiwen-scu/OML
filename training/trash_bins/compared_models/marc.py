...
from model_backbones.marc import MARC
...
class ModelSet():
    def __init__(self, args, test=False):
        ...
        self.model = MARC(1,1).to(device, dtype=torch.float)
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs+1):
            ...
            self._run_epoch_supmotion(epoch, dataloader_sup)
    ...
    def _run_epoch_supmotion(self, epoch, dataloader):
        step = 0
        epoch_loss = 0
        ...
        for index, (still, with_motion, _shotname) in loop:
            ...
            residual_images = with_motion -y_train

            predicted_residuals = self.model(with_motion)
            loss = self.criterion(predicted_residuals, residual_images)

            x_train_rec = with_motion - predicted_residuals
            ...
            if step == 1:
                ...
                self.plotter.image('with_motion', epoch, with_motion)
    ...
    def _validate(self, epoch, val_loader):
        ...
        with torch.no_grad():
            ...
            for index_val, (still_test, withmotion_test, _shotname) in val_loop: #  增加GPU 利用率
                ...
                residuals = x_test-y_test

                predicted_residulas = self.model(x_test)
                loss_val = self.criterion(predicted_residulas, residuals)
                x_test_rec = x_test - predicted_residulas
