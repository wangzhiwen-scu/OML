...
import sys
sys.path.append('.') # 
from layers.mask_layer import Hand_Tailed_Mask_Layer, Hand_Tailed_Mask_Layer4SSL, ComplexAbs
from model_backbones.dualdomain import DualDoRecNet4SSL as DualDoRecNet, fft, ifft
from model_backbones.ista_net import ISTANetplus, ista_net_loss, rfft
...
def loss_func_ssl(u_k, mask, mask1, mask2):
    ...
    uu_img1_abs = torch.norm(uu_img1, dim=1, keepdim=True)
    uu_img2_abs = torch.norm(uu_img2, dim=1, keepdim=True)
    
    pred1, symloss_1 =  model(uu_img1_abs, mask1)
    pred2, symloss_2 =  model(uu_img2_abs, mask2)

    comp_mask1 = full_mask - mask1
    comp_mask2 = full_mask - mask2

    comp_img1 = ifft_func(rfft(pred1) * comp_mask1)
    comp_img2 = ifft_func(rfft(pred2) * comp_mask2)


    # loss_res = 1/2*(mse(comp_img1, pred2)+mse(uu_img2, pred1))


    # pred_k =  fft_func(model(u_k, mask))
    # loss_mc = mse(pred_k, u_k)
    comp_img1 = torch.norm(comp_img1, dim=1, keepdim=True)
    comp_img2 = torch.norm(comp_img2, dim=1, keepdim=True)

    loss = mse(comp_img1, comp_img2) + ista_net_loss(pred1, symloss_1, uu_img1) + ista_net_loss(pred2, symloss_2, uu_img2)
    # loss = mse(pred_k, u_k)

    # u_img = fft_func(u_k)
    # pred =  u_img - model(u_k, mask)
...
def test(model, u_k, clean_img, mask):
    ...
    with torch.no_grad():
        u_img = ifft_func(u_k)
        u_img_abs = torch.norm(u_img, dim=1, keepdim=True)
        [pred, ista_feature] =model(u_img_abs, mask)
