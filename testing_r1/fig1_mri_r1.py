...
from training.mri_py.main_ei_tta import ModelSet as EI
from training.mri_py.main_mdrecon import ModelSet as MDRNet
from training.mri_py.main_metainvnet import ModelSet as Metainvnet
from training.mri_py.main_ttt_tta import ModelSet as TTT
...
def test():
    ...
    df = {'ZF': None, 'EI': None, 'MDReconNet': None, 'TTT': None, 'MetaInvNet': None, 'Ours':None}
    ...
    with torch.no_grad():
        for x_iter in dataloader:
            ...
            output = metainvnet_model(out['zero_filling'], out['batch_trajectories'])
            pred_results, _layers_sym, _layers_st = output
            pred_results = torch.norm(pred_results, dim=1, keepdim=True)
            imgs['MetaInvNet'] = pred_results.detach().cpu().numpy()[0, 0, ...]
